...
1.  **Crie o arquivo de código:** Escreva seu código na linguagem do compilador e salve-o com a extensão `.hahaha`. Por exemplo: `meu_programa.hahaha`. Não se esqueça de referenciá-lo no arquivo make.

2.  **Execute o comando `make`:** No terminal, navegue até o diretório onde o compilador e seu arquivo `.hahaha` estão localizados e execute o comando `make`. Este comando se encarregará de compilar o compilador (se necessário) e executar o seu programa.
...
### 4. String com Entrada Padrão (`stdin`)
...
### 5. Controle de Laço (`continue` e `break`)
...
### 7. Operadores Unários e Compostos
...
### 8. Laço `foreach` (Sintaxe Simplificada)
...
### 9. Vinculação Dinâmica de Tipos
...
### 10. `break all` (Saída de Laços Aninhados)
